...
DEFAULT_REPORT_TYPE = "医生端问卷调研分析报告"
...
def parse_markdown_content(path: Path) -> tuple[dict, dict]:
    ...
    def flush_item() -> None:
        ...
        if not current_item:
            return
        current_item["body"] = normalize_space(" ".join(current_item.pop("lines")))
        if current_section == "data_analysis":
            data_items.append(current_item)
        elif current_section == "suggestions":
            suggestion_items.append(current_item)
        current_item = None
    ...
    for line in lines:
        ...
        if heading:
            ...
            if current_section == "data_analysis" and level >= 3:
                ...
                continue
            if current_section == "suggestions" and level >= 3:
                flush_item()
                current_item = {"title": clean_dimension_title(title), "lines": []}
                continue
        ...
        if current_section == "introduction":
            ...
            if current_item:
                ...
                sug_section_lines.append(line)

    flush_item()

    if not suggestion_items and any(line.strip() for line in sug_section_lines):
        suggestion_items = parse_label_body_blocks(sug_section_lines, "suggestions")
    else:
        suggestion_items = [{"title": item["title"], "body": item["body"]} for item in suggestion_items]

    content = {
        "introduction": parse_paragraphs(intro_lines),
        "background": parse_paragraphs(background_lines),
        "data_analysis": [{"title": item["title"], "analysis": item["body"]} for item in data_items],
...
def parse_jsonl_content(path: Path) -> tuple[dict, dict]:
    ...
    for line_no, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        elif line_type == "background":
            content["background"].append(normalize_space(item["text"]))
        ...
        elif line_type == "suggestions":
            content["suggestions"].append({"title": clean_dimension_title(item["title"]), "body": normalize_space(item["body"])})
        else:
...
def build_payload(questionnaire: dict, meta: dict, content: dict, cli_args: argparse.Namespace) -> dict:
    ...
    if not content["background"]:
        raise ValueError("Background section is empty.")
    ...
    if not content["suggestions"]:
        raise ValueError("Suggestions section is empty.")
    ...
    if cli_args.cover_template_docx:
        payload["cover_template_docx"] = cli_args.cover_template_docx
    return payload
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build a valid doctor survey report payload JSON from questionnaire JSON and structured report content.")
    ...
    parser.add_argument("--cover-template-docx", help="Optional cover template docx path")
