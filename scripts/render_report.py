...
def add_labeled_body_para(doc: Document, label: str, body: str, first_indent_pt: int = 28):
    p = doc.add_paragraph()
    p.paragraph_format.line_spacing = 1.5
    p.paragraph_format.space_before = Pt(8)
    p.paragraph_format.space_after = Pt(0)
...
def write_docx(payload: dict, output: Path, charts_dir: Path):
    ...
    add_secondary_heading(doc, "1.1 报告背景")
    ...
    for item in payload["positive_feedback"]:
        add_labeled_body_para(doc, item["title"], item["body"], first_indent_pt=21)
    add_secondary_heading(doc, "3.2 待改进反馈")
    for item in payload["negative_feedback"]:
        add_labeled_body_para(doc, item["title"], item["body"], first_indent_pt=21)
    add_primary_heading(doc, "4、优化建议")
    for item in payload["suggestions"]:
        p = add_secondary_heading(doc, f"· {item['title']}")
        add_body_para(doc, item["body"])
    add_primary_heading(doc, "5、附件-问卷题目内容")
    for item in payload["attachment_questions"]:
        p = doc.add_paragraph()
        p.paragraph_format.space_before = Pt(8)
        p.paragraph_format.space_after = Pt(0)
